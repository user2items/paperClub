...
if MODEL is None:
    MODEL = load_learner('coco2/models', 'logo_detect.pkl')
...
def plot_bbox(img, scores_tot, bboxs_tot,labels_tot, is_xyxy=False):
    ...
    if bboxs_tot:
        for i, bbox_pred in enumerate(bboxs_tot[0]):
            ...
            cv2.putText(img_copy, pre_str, (x1, int(y1 - 0.3 * text_height)), cv2.FONT_HERSHEY_SIMPLEX, 0.45, TEXT_COLOR, lineType=cv2.LINE_AA)
...
src = cv2.imread('F:/NSFW/watermark_detection/new_data/images/00c1a0f7074f1b5b_9-3297_2_610_610.jpg')
img = image_pad(src)
img_pad_pil = PIL.Image.fromarray(cv2.cvtColor(img,cv2.COLOR_BGR2RGB))
img_pad_in = PIL2Tensor(img_pad_pil)
bboxs_tot,scores_tot,labels_tot = do_inference(img_pad_in)
# print(bboxs_tot,scores_tot,labels_tot)
# xyxys = Bbox2XYXYs(bboxs_tot)
# print("xyxys:",xyxys)
# new_xys = list(map(Scale2Coords,[[img.shape[:2],torch.Tensor([x]), src.shape[:2]] for x in xyxys]))
# print("new_xys:",new_xys)

predict_api('F:/NSFW/watermark_detection/new_data/images/00c1a0f7074f1b5b_9-3297_2_610_610.jpg',is_show=True)
